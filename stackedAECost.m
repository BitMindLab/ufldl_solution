...
% 
% % 2. softmax layer
% softmaxCost = -1/numCases*sum(sum(groundTruth.*log(a4))) + 0.5*lambda*sum(sum(softmaxTheta.^2));
% softmaxThetaGrad = -1/numCases*(groundTruth - a4)*data' + lambda*softmaxTheta;
% 
% 
% % 3. autoencoder layer
% p = sum(a2,2) / m;
% q = sparsityParam;
% kl = KL(q,p);  
% autoencoderCost = 0.5 / m * sum(sum((a3 - data).^2)) + 0.5 * lambda * (sum(sum(W1.^2)) ...
...
% 
% cost = softmaxCost + autoencoderCost;

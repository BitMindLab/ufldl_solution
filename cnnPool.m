...
for i = 1:floor(convolvedDim / poolDim)
    for j = 1:floor(convolvedDim / poolDim)
        dimi = [1:poolDim] + (i-1)*poolDim;
        dimj = [1:poolDim] + (j-1)*poolDim;
        A = convolvedFeatures(:,:,dimi,dimj);
        A = reshape(A,numFeatures,numImages,poolDim*poolDim);
        pooledFeatures(:,:,i,j) = mean(A, 3);
    end
end
